...
my $dbh = DBI->connect("dbi:Proxy:hostname=localhost;port=3333;dsn=DBI:XBase:.")
	or die $DBI::errstr;

$dbh->do("drop table list");
$dbh->do("drop table rank");
...
$sth = $dbh->prepare("update list set name = list.name || '!'  where id < 20; ");
#$sth = $dbh->prepare("update list set name = list.name || '!' where id in (select id from rank where perl > 90)");
$sth->execute();
